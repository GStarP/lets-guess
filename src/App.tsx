...
import { LoadShareDialog } from "./components/load-share-dialog";
...
function App() {
  ...
  useEffect(() => {
    const shareUrl = extractShareUrlFromHash();
    if (shareUrl) {
  ...
  const resultEntries = useMemo(() => {
    ...
    return session.items.map<ResultEntry>((item) => {
      const selectedName =
        session.selectedNameByImageId[item.imageId] ?? "未选择";
      ...
      return {
        ...
        reason,
        isCorrect:
          normalizeName(selectedName) === normalizeName(item.correctName),
      };
    });
    ...
    >
      {view === "setup" ? (
        <SetupView
          ...
          onShareError={(msg) => pushMessage({ type: "error", text: msg })}
        />
      ) : null}
      ...
      {pendingShareUrl ? (
        <LoadShareDialog
          ...
          onError={(msg) => pushMessage({ type: "error", text: msg })}
        />
      ) : null}
